...
def parse_and_validate_structured_output(raw_output: str) -> StructuredLegalResponse:
    ...
    json_match = re.search(r'``c(?:json)?\s*(${+[\s\S]*?})\s+```', clean_text)
    if json_match:
...
def verify_citations_and_claims(
    ...
    for claim in response.claims:
        ...
        if not matched_entry:
            ...
            claim.verification_reason = f"Source ID '{claim.source_id}' not found in retrieved turn context"
            failures.append({
                "query": query,
                "claim_text": claim.text,
                "attempted_source_id": claim.source_id,
                "quoted_text": claim.quoted_text,
                "failure_type": "SOURCE_ID_NOT_RETRIEVED",
                "retrieved_source_ids": [c.get("gr_no") or c.get("title") or c.get("doc_id") for c in retrieved_chunks]
            })
            continue
        ...
        claim.verified = True
        claim.verification_reason = "Verified against retrieved context"
        verified_count += 1

    for claim in response.claims:
        if claim.verified is False:
            flag_badge = f'<span class="unverified-citation text-rose-600 font-bold" title="[UNVERIFIED: {claim.verification_reason}]">⚠️ [{claim.source_id} (Unverified)]</span>'
        ...
        try:
            with open(VERIFICATION_LOG_PATH, "a", encoding="utf-8") as f:
                for fail in failures:
                    fail["timestamp"] = datetime.utcnow().isoform()
